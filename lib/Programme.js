module.exports = class Programme {
    ...
    async getEventsForWeek(date, callback, fallbackOnSeason=true) {
    ...
    async getEventsForSeason(season, date, callback) {
        ...
        if ((events) && (events.length > 0)) {
            var programmeMonMidnight = Programme.startOfWeek(date);
            var seasonMonMidnight = Programme.startOfWeek(events[0].start);
            var days = Math.ceil((programmeMonMidnight.getTime() - seasonMonMidnight.getTime()) / (24 * 60 * 60 * 1000));
            events = events.filter(e => (e.id != 0)).map(e => {
                e.start =  new Date(e.start); e.start.setDate(e.start.getDate() + days); e.start = e.start.toISOString();
                e.end =  new Date(e.end); e.end.setDate(e.end.getDate() + days); e.end = e.end.toISOString();
                e.seasonEvent = true;
                return(e);
            });
        }
