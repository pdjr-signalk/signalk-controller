...
module.exports = class Controller extends Schedule {
    ...
    constructor(options) {
        ...
        this.state = {
            channels: {},
            overrides: { modulate: false, frost: false }
        };

        this.options.channels.forEach(channel => { this.state.channels[channel.name] = { events: [], mode: channel.defaultMode }; });
    ...
    toggleOverride(override) {
        if (this.options.debug) console.log("Controller.toggleOverride(%s)...", override);

        if (this.state.overrides.hasOwnProperty(override)) {        
            console.log("HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH");
            this.state.overrides[override] = (! this.state.overrides[override]);
            this.notifyState();
        }
    }

    getChannelsInGroup(group, callback) {
        if (this.options.debug) console.log("Controller.getChannelsInGroup(%s)...", group);

        callback(this.options.channels.filter(channel => (channel.group == group)));
    }
    ...
    notifyState() {
        if (this.options.debug) console.log("Controller.notifyState()...");

        Object.keys(this.state.overrides).forEach(key => {
            this.options.issueNotificationCallback("controller." + key, "normal", (this.state.overrides[key])?"ON":"OFF");
        });
    }
        ...
        var protocol = {
            ...
            verbs: {
                getChannelsInGroup: {
                    requires: [ 'group' ],
                    returns: 'channels',
                    func: function(params, callback) { this.getChannelsInGroup(params.group, callback); }.bind(this)
                },
                setChannelMode: {
                ...
                toggleOverride: {
                    requires: [ 'override' ],
                    returns: null,
                    func: function(params) { this.toggleOverride(params.override); }.bind(this)
                }
            }
        }
