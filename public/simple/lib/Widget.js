...
class IndicatorComponent extends WidgetComponent {
    ...
    constructor(parentNode, params) {
        ...
        this.params = IndicatorComponent.setParameterDefaults(params);
        this.valueNode = null;

        //var div = document.createElement("div");
        //div.className = "widget-component " + ((notification)?"widget-notification":"widget-indicator");
        var match = this.node.innerHTML.match(/(.*)---(.*)/);
        if (match) {
            this.valueNode = document.createElement("span");
            var prefix = (match.length >= 2)?match[1].slice(0):"";
            var suffix = (match.length >= 3)?match[2].slice(0):"";
            this.node.innerHTML = "";
            this.node.appendChild(document.createTextNode(prefix));
            this.node.appendChild(this.valueNode);
            this.node.appendChild(document.createTextNode(suffix));
        }
        var rafunc = super.resetAnimation;
        super.setUpdateFunction(function(v) {
            if (v) {
                if (this.valueNode) this.valueNode.innerHTML = this.params["onvalue"];
                this.node.classList.remove(this.params["offclass"]);
                this.node.classList.add(this.params["onclass"]);
            } else {
                if (this.valueNode) this.valueNode.innerHTML = this.params["offvalue"];
                this.node.classList.remove(this.params["onclass"]);
                this.node.classList.add(this.params["offclass"]);
            }
        }.bind(this));
    }

    static setParameterDefaults(params) {
        if (!params.onvalue) params.onvalue = "";
        if (!params.offvalue) params.offvalue = "";
        if (!params.onclass) params.onclass = "widget-indicator-on";
        if (!params.offclass) params.offclass = "widget-indicator-off";
        return(params);
    }
